            ...
            <div className="md:hidden flex items-center gap-2">
              {isAuthenticated && (
                <>
                  <Link
                    to="/dashboard"
                    className="px-3 py-1.5 rounded-full border border-landrify-green/40 text-landrify-green text-xs font-bold uppercase tracking-wider hover:bg-landrify-green/10 transition"
                  >
                    Dashboard
                  </Link>
                  <button
                    onClick={() => setProfileOpen(true)}
                    className="relative w-9 h-9 rounded-full bg-landrify-green text-white flex items-center justify-center font-bold text-sm shadow-md"
                    aria-label="Open profile"
                  >
                    {initial}
                    {user?.nin_verified && (
